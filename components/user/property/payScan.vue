<template>
  <Row class="payScan_main_body">
    <!-- <p>选择支付渠道：</p> -->
    <Form :model="formData" :label-width="150" class="pay_form" :rules="formRule" ref="thisForm" v-show="!stepTwoShow">
      <FormItem label="选择支付渠道：" prop="radio">
      ...
      <FormItem label="充值账号:" prop="account" class="form_price" v-if="accountIsShow">
        <Input v-model="formData.account" placeholder="请输入充值账号"></Input>
      </FormItem>
      <FormItem label="充值金额:" prop="price" class="form_price">
        <Input v-model="formData.price" placeholder="请输入充值金额" :number="true"></Input>
        <!-- <AppInputNumber class="number " v-model="formData.price"></AppInputNumber> -->
      </FormItem>
      <Row style="text-align:center;">
        <Button size="large" type="primary" @click.native="confirmPay" :loading="submitting">确认充值</Button>
      </Row>
    </Form>
    <Row class="payScan_step_two" v-show="stepTwoShow">
      <Row class="payScan_step_two_top">
        <Col :span="4" class="left">
          <Icon type="checkmark-round" style="font-size: 75px;color:rgb(34,172,56)"></Icon>
        </Col>
        <Col :span="20" class="right">
          <p>订单提交成功，请扫描以下二维码付款！</p>
          <p class="order">订单号：<span>{{order}}</span></p>
          <p class="price">付款金额：<span>{{price}}元</span></p>
          <p v-if="accountIsShow">入款账号：{{account}}</p>
          <p v-if="accountIsShow">入款昵称：{{nickName}}</p>
        </Col>
      </Row>
      ...
      <Row class="payScan_step_two_bottom">
        <Button type="primary" size="large" @click.native="stepTwoShow=false">返回</Button>
        <Button type="primary" size="large" @click.native="getBack">我已付款</Button>
      </Row>
    </Row>
    <p v-html="tips" class="payScan_tips"></p>
  </Row>
</template>
<script>
    ...
    computed: {
      ...
      accountIsShow() {
        if(this.basicData.length > 0) {
          let data = this.basicData.find((item, index) => index == this.formData.radio)
        ...
        this.$refs.thisForm.validate(async(valid) => {
          if(valid) {
            ...
            if(this.accountIsShow) {
              let manData = this.basicData.find((item, index) => index ==this.formData.radio)
              this.account = manData.account
              this.nickName = manData.name
