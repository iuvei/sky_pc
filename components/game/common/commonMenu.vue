<template>
    <div class="common-menu " v-switchClass="{key:$route.params.id,class:'shake'}">
        ...
        <div class="bet-submenu" v-if="data.length && $route.name.split('-')[1] != 'xync'">
...
export default {
  ...
  computed: {
    data() {
      ...
      const singleArr = ['k3', 'lhc', 'pcdd', 'xync','xypk'],
        multiArr = ['ssc', '11x5', 'pk10', 'f3d', 'qxc']
  ...
  methods: {
    ...
    resetActive() {
      ...
      this.$nextTick(() => {
        this.playId = this.data[this.menuIdx].submenu[0].playlist[0].playid
      })
    },
    switchOfficial(k) {
