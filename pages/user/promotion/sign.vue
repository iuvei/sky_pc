<template>
  <div class="sign_body">
    ...
    <Tabs :animated="false" @on-click="toGetUserAwardLog" v-model="nowTab">
      <TabPane label="每日签到">
        <div class="month">
          <span>{{currentMonth}}</span>月签到</div>
        <div class="month_box">
          <div class="weekdays">
            <ul>
              <li>星期日</li>
        ...
        <div class="bxyl_right">
          ...
          <div class="box">
            <p class="title">累计签到天数:
              <span class="sign_number">{{signdays}}</span>天</p>
            <div id="win_box" style="padding: 0 20px;">
              <div class="box_cent">
                <div class="ling_box">
                  <div class="box_list">
                    <p class="_title">累计签到
                      <span style="color:red;">{{totaldays}}</span> 天可以领取</p>
          ...
          <div class="reward_box" v-for="(item,index) in awardlog" :key="index" v-show="(offset<=index&&index<(limit+offset))">
            ...
            <div class="cont">
              <p class="money">成功领取
                ...
                <span>{{item.reward_price}}</span>元</p>
              <p class="day">已达到
                <span>{{item.sign_days}}</span>天奖励资格</p>
            </div>
            <div class="bag">
    ...
    <Modal v-model="modal7" :closable="false">
      <p class="baoxiang"><img src="~/assets/img/baoxiang_ok.png" alt="">
        ...
        <p class="awarddetail">获得
          <span style="font-size:60px;color:rgb(235,51,73)"> {{rewardprice}} </span>元
          <span>{{rewardmode}}</span>
        </p>
