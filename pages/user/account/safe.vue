<template>
	<div class="safe_body">
		<div class="safe_level">
			...
			<div class="line_2">上次登录: 2018-05-07 15:21:39 | 不是我登录?
		...
		<div class="box_container" id="dialog_userinfo">
			...
			<div class="content">
				<div class="item no_select">
					...
					<div class="_content">
						...
						<div class="_info">建议您使用字母和数字的组合、在组合中加入下划线。</div>
					</div>
					<div class="_tools">
			...
			<div class="loginpass" v-show="nameBox">
				...
				<div class="modify">
					...
					<!-- <div class="box">
						<span class="tit">确认姓名</span>
						<Input v-model="new_name2" placeholder="请输入真实的姓名" clearable style="width: 200px"></Input>
					</div> -->
          <p style="text-align:center;color:#666;padding:5px 0">请填写真实的姓名与便于提款验证</p>
            ...
            <Input type="text" v-model="vcode" placeholder="验证码..." :maxlength="vcodeLength" clearable class="w110 vcode" @on-focus="randomVerify1" style="display: inline-block">
              <div slot="append" style="overflow:hidden;width:60px;height:30px;" @click="randomVerify1">
                <!-- <img :src="verifyImg" alt="" class="vscode-locked-outline"> -->
                <span @click="randomVerify1" ref="randomVerifyImg1" style="display: inline-block;width: 100%;height: 100%;">
                  <i v-for="(item, index) in verifyArr" :key="index" style="color: white">{{item}}</i>
						...
						<!-- <Input v-model="vcode" placeholder="请输入验证码" clearable style="width: 200px" class="code"></Input>
            <img :src="verifyImg" alt="" @click="getVerify" class="verifyImg"> -->
					</div>
				</div>
  ...
  methods: {
    ...
    updateInfo(type) {
      ...
      if (type === 3) {
        ...
        this.update.vid = this.vid;
        this.update.vcode = this.vcode;
        this.getVerify()
      } else if (type === 1) {
        if(this.is_name){
    ...
    correctPW() {
      ...
      this.setTradPassArr.tkpass_ok = this.userinfo.tkpass_ok;
